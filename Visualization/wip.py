...
def heatmap(episode, dataset_name, train_test):
    ...
    df = df[(df['Episode'] == str(episode))] # -1 is E-Greedy
    # print(df['Choice'][df['Price Delta'].astype(float) > 2])
    price_min, price_max = min(df['Price Delta']), max(df['Price Delta'])
    volume_min, volume_max = min(df['Volume Delta']), max(df['Volume Delta'])
    # x = np.arange(price_min, price_max, .2).tolist()
    # y = np.arange(volume_min, volume_max, 30).tolist()
    ...
    x = x[1].to_tuples()
    y = y[1].to_tuples()
    # x = pd.qcut(df['Price Delta'], q=20, retbins=True)
    # y = pd.qcut(df['Volume Delta'], q=20, retbins=True)
    # print(pd.qcut(df['Price Delta'], q=10))
    ...
    fig = go.Figure(go.Heatmap(z=new_df, x=x[1], y=y[1], colorscale=[[0, '#D3D3D3'],[.5, '#ffdb58'], [.75, '#66CDAA'], [1, '#f88379']]))
    # fig = px.imshow(new_df, x=x[1], y=y[1], color_continuous_scale='Inferno')
    fig.update_layout(
